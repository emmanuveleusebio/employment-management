...
//* form validation >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  if(password.value == ""){
      ...
      errorMsg[7].innerText = "Password must include special character and number";
      errorMsg[7].style.display = "block";
      return;
  } else {
...
console.log(newUserDetails)
  ...
  .then((data) => {
     ...
     console.log('this sifkdfdsffjffffff', data._id)
...
// document.querySelectorAll('input[type="text"], input[type="email"], input[type="tel"], input[type="password"], input[type="date"], select').forEach(inputField => {
//   inputField.addEventListener('input', () => {
//       const errorMsg = inputField.parentElement.querySelector(".errorMsg");
...
// document.querySelectorAll('input[name="gender"]').forEach(function(radioBtn) {
//   radioBtn.addEventListener('click', function() {
//       document.querySelector(".genderNull").style.display = "none";
//   });
// });
// phone.addEventListener('input', function() {
//   const errorMsg = this.parentElement.querySelector(".errorMsg");
//   errorMsg.style.display = this.value.length === 10 ? "none" : "block";
// });
...
function editPage(id,event) {
  ...
  event = event || window.event; // For compatibility with older versions of Internet Explorer
  if (event) {
      event.preventDefault(); // Prevent default action if event object exists
  }
...
searchBar.addEventListener('input', async ()=> {
    ...
    try {
        const response = await fetch(`http://localhost:5001/api/user/search?search=${searchFor}`);
